...
void display(int sx,int sy)	//Display grid
{
    int i,j;
    char str[2];
    for(i=0;i<3;i++)
...
int checkWin(int sx,int sy,int player)		//Check win; return 1 if any one of two players win else 0
{
    char str[25];
    int i, j;
    for(i=0;i<3;i++)
...
int checkDraw(int sx,int sy)		//Check draw; return 1 if draw else 0
{
    int i, j, k=0;
    char str[25];
    for(i=0;i<3;i++)
...
void main()
{
	...
	switch(ch)
	{
		...
		break;
	    case 32:		//Space key
		if(grid[sy][sx]==32)       //Mark the cell only if it is empty
		{
		    if(player==0)
			grid[sy][sx]='X';
		    else
